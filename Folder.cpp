...
{
   size_t total_size = 0; // Initialize a variable to store the total size
   // Iterate over each file in the files_ vector
   for(const auto& list : files_) 
   {
      // Accumulate the size of each file by calling its getSize() method
      total_size += list.getSize();
   }
...
{
    // If the file's name is empty, it means the file has already been moved, so we cannot add it
    if (new_file.getName().empty()) 
    {
        return false;
    }
    // Loop through the files_ vector to check if a file with the same name already exists
    for (size_t i = 0; i < files_.size(); ++i) 
    {
        if (files_[i].getName() == new_file.getName()) 
        {
            // If a file with the same name exists, return false (duplicate files are not allowed)
            return false;
        }
    } 
    // Use move semantics to avoid unnecessary copying and efficiently transfer ownership of the file 
    files_.push_back(std::move(new_file));
    // Return true indicating the file was added successfully
    return true; 
}
...
{
    // If the filename is empty, there's nothing to remove
    if (name.empty()) 
    {
        return false;
    }
    // Iterate through the vector to find the file with the matching name
    for (size_t i = 0; i < files_.size(); ++i) 
    {
        if (files_[i].getName() == name) 
        {
            // If we find the file, erase it from the vector and return true
...
{
    // If the source and destination folders are the same, no need to move the file
    if (this == &destination) 
    {
        // If they are the same, there's no need to move the file, so return true
        return true; 
    }
    // Find the file in the current folder by its name
    auto file_it = std::find_if(files_.begin(), files_.end(), [&name](const File& file) { return file.getName() == name; });
    // If the file is not found in the current folder, return false
    if (file_it == files_.end()) 
    {
        return false;
    }
    // Check if a file with the same name already exists in the destination folder
    bool already_exists = std::any_of(destination.files_.begin(), destination.files_.end(),[&name](const File& file) { return file.getName() == name; });
    // If a file with the same name exists in the destination, return false
    if (already_exists) 
    {
        return false;
    }
    // Move the file from the current folder to the destination folder using move semantics
    destination.files_.push_back(std::move(*file_it));
    // Erase the moved file from the current folder
...
bool Folder::copyFileTo(const std::string& name, Folder& destination) 
{
    ...
    // Check if a file with the same name already exists in the destination folder
    bool name_conflict = std::any_of(destination.files_.begin(), destination.files_.end(), [&name](const File& file) { return file.getName() == name; });
    // If a file with the same name already exists, return false
    if (name_conflict) 
    {
        return false; 
    }
    // Use the copy constructor or assignment operator to create a deep copy of the file in the destination folder
    destination.files_.emplace_back(*file_it);
    // Return true indicating the file was successfully copied
    return true; 
}
